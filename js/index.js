...
async function fetchPokemons(){
  const response = await fetch("https://pokeapi.co/api/v2/pokemon?limit26&offset=1");
...
function insertPokemonsHtml(pokemon){
    ...
    <li class="pokemons-item">
      <div class="pokemons-card" data-pokemon-card-type-name="eletric">
        ...
        <ul class="pokemons-card-attributes">
          ...
          <li class="pokemons-card-attributes-item">
            <span class="pokemons-card-attributes-name">Special Attack</span>
            <div class="pokemons-card-attributes-progress">
...
async function populatePokemons(pokemons) {
  ...
  for (const allPokemonData of allPokemonsData) {
    insertPokemonsHtml(allPokemonData)  
  }
}
  ...
  if(value === ""){
    ...
    if(currentPokemon){
      ...
      if(currentPokemonData){
        removeAllPokemons()
        insertPokemonsHtml(currentPokemonData)
      }
    } else {
